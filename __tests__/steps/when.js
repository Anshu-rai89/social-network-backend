...
const we_invoke_getImageUploadUrl = async (userName, extension, contentType)=> {
   const getImageUrlHandler = require("../../functions/get-signed-url.js").handler,
    context = {},
    event = {
      identity: {
        userName
   ...
   return await getImageUrlHandler(event, context);
}

const a_user_calls_getImageUpload = async (user, extension, contentType) => {
  const getImageUploadUrl = `
    query MyQuery($extension: String, $contentType: String){
  ...
  console.log(`[${user.userName} image upload url fetched]`);
